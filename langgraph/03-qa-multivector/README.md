...
- 可选环境变量：`PG_CONNECTION`、`QA_MULTIVECTOR_COLLECTION`（默认 `qa_multivector`）、`QA_MULTIVECTOR_DOCSTORE`（默认 `langgraph/03-qa-multivector/docstore.json`）、`QA_MULTIVECTOR_RETRIEVE_K`（默认 4）。
...
```bash
uv run python langgraph/03-qa-multivector/build_db.py <文档目录>
```

示例：

```bash
uv run python langgraph/03-qa-multivector/build_db.py langgraph/02-qa/docs
```

## 步骤二：测试检索

```bash
uv run python langgraph/03-qa-multivector/test_retrieval.py "项目里有哪些 demo？"
```
...
```bash
uv run streamlit run langgraph/03-qa-multivector/app.py
```

与 02-qa 类似，多轮提问；底层使用 `MultiVectorRetriever`，返回的是父文档。
