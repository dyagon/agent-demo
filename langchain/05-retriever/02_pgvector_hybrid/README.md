# 02_pgvector_ensemble：EnsembleRetriever
...
与 03-embedding / 03-vector 共用表，请先建表并灌数：
...
# 自定义权重（向量 0.7，全文 0.3）
uv run python langchain/04-retriever/02_pgvector_ensemble/app.py "你的问题" --weights 0.7 0.3

# 指定返回条数
uv run python langchain/04-retriever/02_pgvector_ensemble/app.py "你的问题" -k 8
```
