...
│   │   ├── README.md
│   │   └── app.py
│   └── 03-rag/         # PGVector 混合检索（向量 + tsvector）
│       ├── README.md
│       ├── config.py
│       ├── schema.sql
│       ├── build_db.py
...
└── langgraph/          # LangGraph 示例
    ...
    │   ├── app.py
    │   └── docs/
    └── 03-qa-multivector/  # 多向量检索：摘要向量 + docstore 父文档
        ├── README.md
        ├── config.py
        ├── file_docstore.py
        ├── build_db.py
...
| langchain/03-rag | PGVector 混合检索：先执行 schema.sql 建表，build_db.py 建库，query.py --mode vector / hybrid 对比 | 见 `langchain/03-rag/README.md` |
...
| langgraph/03-qa-multivector | 多向量检索：摘要向量 + docstore，`build_db.py <文档目录>` 建库后 `streamlit run app.py` 对话 | 见 `langgraph/03-qa-multivector/README.md` |
